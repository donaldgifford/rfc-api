...
- [x] **Index inventory** — deferred as YAGNI. The live corpus is <100 documents (one row per docz-managed file across rfc-api + future-feeder repos); sequential scan latency is sub-millisecond at this scale. Existing indexes on `documents`: `(created_at DESC, id ASC)`, `(type, created_at DESC, id ASC)`, plus PK on `id`. New sorts (`created_asc`, `updated_desc/asc`, `id_asc/desc`) and their filtered variants would each need their own index for a strict index-scan, but adding 6+ indexes preemptively trades small write-amplification for a benefit that never materializes at this scale. Reopen if a future EXPLAIN ANALYZE on a real workload shows a seq-scan bottleneck — at that point a single targeted `(updated_at DESC, id ASC)` migration (the most-likely-to-be-hit sort if rfc-site adopts "Most Recently Updated") is the natural next step. Decision aligns with the project's "no design for hypothetical future requirements" rule.

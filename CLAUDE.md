...
[IMPL-0006][impl-0006] is **Ready to ship** (code-side phases 1–3 complete, Phase 4 = release tag + reindex + rfc-site coordination, all user-driven, 2026-05-08). Consumer-side slug contract — porting `github-slugger` semantics so `rfc-api`'s sub-doc slugs match what `rfc-site`'s `rehype-slug` plugin renders for the same Markdown:
...
- `test/contract/slug_test.go` + `test/contract/testdata/slug_fixtures.json` — snapshot-fixture contract test. The snapshot is regenerated from pinned `github-slugger@2.0.0` via `make regen-slug-fixtures` (wraps `scripts/regen-slug-fixtures.sh`). To bump upstream, edit `SLUGGER_VERSION` in the script and commit the regenerated fixture + the version bump in one commit — never silently. rfc-site re-runs an equivalent fixture against the same pinned upstream version on its side; the two repos triangulate on upstream rather than copy-blindly.
- See [INV-0002][inv-0002] for the divergence inventory (50% synthetic divergence, ~18% of the live corpus broken). Operator runbook for the Phase 4 rollout (reindex, rfc-site rerun, drift check, issue #20 close) lives in IMPL-0006 Phase 4 — paste verbatim into the PR body when opening the merge PR.

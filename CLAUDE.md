...
[IMPL-0006][impl-0006] is **In Progress** (Phases 1–3 complete, 2026-05-08). Consumer-side slug contract — porting `github-slugger` semantics so `rfc-api`'s sub-doc slugs match what `rfc-site`'s `rehype-slug` plugin renders for the same Markdown:
...
- See [INV-0002][inv-0002] for the divergence inventory (50% synthetic divergence, ~18% of the live corpus broken). Phase 4 (reindex + rfc-site coordination + issue #20 close) still pending.
